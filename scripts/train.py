...
def run_mlflow_complete_experiment(parquet_path: str):
    """🔬 NOVO: Execução completa com tracking MLflow para paper."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    print("🔬 EXPERIMENTO COMPLETO PARA PAPER")
    print("🎯 COM TRACKING MLFLOW PROFISSIONAL")
    print(f"Device: {device}")
    print("=" * 60)
    ...
    model, train_losses, val_losses = train_humob_model(
        ...
        device=device,
        cities=["A"],
        n_epochs=8,  # Mais épocas para melhor resultado
        learning_rate=1e-3,
        batch_size=32,
        sequence_length=24,
        n_users=100_000,
...
def main():
    ...
    print("🔬 MLflow para tracking acadêmico!")
    print()
    ...
    print("🔬 Tracking completo com MLflow para paper")
    print("🔧 Compatibilidade PyTorch 2.6+ corrigida")
    print()
    ...
    print("8. 🎓 Experimento COMPLETO para paper (6-12h)")        # 🆕 NOVO
    print("9. 📊 Ver resumo dos experimentos MLflow")              
    print("10. 📈 Exportar dados para paper (tabelas/gráficos)")   # 🆕 NOVO
    ...
    try:
        ...
        # 🆕 NOVA OPÇÃO: Experimento completo
        elif choice == "8":
